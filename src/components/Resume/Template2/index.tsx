...
import {
	...
	CheckCircleFilled, ScheduleFilled, EnvironmentFilled, HeartFilled, CrownFilled, TrophyFilled,
...
export const Template2: React.FC<Props> = ({ value, theme }) => {
	...
	const moduleRenderers: Record<string, () => React.ReactNode> = {
		...
		aboutme: () => aboutme.join('').trim() ? (
			<Wrapper title={titleNameMap.aboutme} className="section-aboutme" color={theme.color}>
		...
		awardList: () => awardList.length > 0 ? (
			<Wrapper title={titleNameMap.awardList} className="section-award" color={theme.color}>
