...
export const Template3: React.FC<Props> = ({ value, theme }) => {
  ...
  const moduleRenderers: Record<string, () => React.ReactNode> = {
    ...
    aboutme: () => aboutme.join('').trim() ? (
      <CardWrapper title={titleNameMap.aboutme} className="section section-aboutme" color={theme.color}>
    ...
    workExpList: () => workExpList.length > 0 ? wrapper({ title: titleNameMap?.workExpList,  color: theme.color })(
      <div className="section section-work-exp">
        {workExpList.map((work, i) => {
          const wt: any = work.work_time;
          const [start = null, end = null] = typeof wt === 'string' ? wt.split(',') : (wt || []);
          return work ? (
    ...
    projectList: () => projectList.length > 0 ? wrapper({ title: titleNameMap?.projectList, color: theme.color })(
  ...
  return (
    <div className="template3-resume resume-content">
      <div className="resume-module-flow">
        {moduleKeys.map(key => (
          <React.Fragment key={key}>{moduleRenderers[key]?.()}</React.Fragment>
        ))}
      </div>
    </div>
  );
